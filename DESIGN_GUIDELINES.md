...
- **Net-worth-lead green/red come from `src/utils/leadColors.js`** (`LEAD_COLOR_RADIANT` / `LEAD_COLOR_DIRE` / `LEAD_COLOR_EVEN` = green-500 / red-500 / gray-400). Both gold graphs, `momentum.js` and `SeriesLivePulse`'s lead line import them rather than re-typing `rgb(34,197,94)`. These are the documented exception to "always use Tailwind classes for colors" below: their consumers are SVG `fill`/`stroke` attributes and measured inline `style` values, where a class cannot apply. Any new advantage-colored surface imports the constants — it does not hand-type the literal, which is how nine copies across five files happened.
...
- **The caption below the map is not decorative, but it IS now conditional — superseded 2026-08-08.** The rule as originally written ("renders every time... unconditionally... do not make conditional") was correct for the OD-only path this section describes, where `building_state` genuinely cannot determine barracks/tier-4/Ancient state (confirmed by direct disproof — see `CONTEXT.md`, "R4.0 decode spike"). It no longer holds unconditionally: once Valve's `radiantBarracksState`/`direBarracksState` props are present, barracks genuinely ARE known and drawn, so the original caption text would be actively false, not just conservative. `DotaMinimap.jsx` now renders one of two captions — `hasBarracks ? 'Towers & barracks — Ancient HP still unknown' : 'Towers only — barracks, base towers & Ancient status unknown'` — never neither. The invariant that survives is **some caption always renders, whichever is currently true**; "never conditional" was the wrong generalization of "never silently absent."
