...
- Square SVG, `viewBox 0 0 300 300`, capped at `max-w-[240px] mx-auto` — Radiant base bottom-left, Dire base top-right, faint gray guide lines (`strokeOpacity 0.3`) showing the three lane paths for orientation only, not data.
- Towers are 10×10 squares: standing = filled (`#22c55e` Radiant / `#ef4444` Dire), destroyed = hollow (`fill="transparent"`, dim `#4b5563` stroke, `opacity 0.5`) — the same "filled = alive, hollow/dim = gone" logic a viewer already expects from any sports/strategy map.
- Base corners get a soft colored circle + a `RAD`/`DIRE` text label (not just color) — small, but real text, not color-only signaling.
