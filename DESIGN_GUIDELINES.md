...
### Tower map (`DotaMinimap.jsx`, R4 — owner-only during verification)

A schematic (not a licensed reproduction of Valve's minimap texture — no reliable/licensed image source was found; see `.claude/specs/live-story-roadmap.md`) tower map, placed directly under the momentum band and above `LiveGoldGraph` — the two other "state read" surfaces sit together, ahead of the graph's *history*. Sourced from `pulse.objectives = { radiant: [top,mid,bot], dire: [top,mid,bot] }` (standing-tower counts, 0-3 per lane, decoded server-side, `api/_buildingState.js`), which is only present at all when confidence is high — no separate loading/low-confidence state to design because an absent field already means "don't render." Redesigned 2026-07-27 from a first pass that read as flat/tacky (bare gray guide lines, plain squares, no terrain) into the richer treatment below — still flat colors throughout, no gradients, per the sitewide rule; "richer" here means more geographically complete (river, road styling, wider spacing), not skeuomorphic.

- Wrapped in a bordered card (`border border-gray-200 dark:border-gray-800 rounded bg-gray-50 dark:bg-gray-950 p-2.5`) — the map is a big enough visual element now to deserve its own contained panel, not a floating raw SVG loose in the sheet.
- A small legend row (filled/hollow swatch pair + "Standing"/"Destroyed" labels) sits above the map — added once towers became diamonds rather than obviously-tower-shaped icons, so the fill convention doesn't need to be inferred.
- Square SVG, `viewBox 0 0 300 300`, capped at `max-w-[240px] mx-auto` — Radiant base bottom-left, Dire base top-right.
- **River**: a flat two-layer stroke (a wider `#1e3a5f` base + a narrower `#3b6493` highlight on top — layering, not an SVG gradient) on a gentle cubic-bezier curve crossing near the map's center at a distinctly different angle than mid lane's own diagonal — matches real Dota's geography (the river crosses mid, it doesn't run parallel to it) and is what most made the first pass look under-detailed.
- **Lane roads**: thicker warm-toned strokes (`#8a7a63`, `strokeOpacity 0.4`, `strokeWidth 5`, round caps/joins) tracing each lane's actual path (up-then-across for top, across-then-up for bot, straight diagonal for mid) — reads as a path, not a debug guide line.
- **Towers are 9×9 diamonds** (a `<rect>` rotated 45°): standing = filled (`#22c55e` Radiant / `#ef4444` Dire), destroyed = hollow (`fill="transparent"`, dim `#57534e` stroke, `opacity 0.55`) — the same "filled = alive, hollow/dim = gone" logic a viewer already expects from any sports/strategy map. A diamond reads as a distinct "structure" glyph rather than a plain data-viz square.
- **Ordering is the load-bearing correctness property, not just a visual one.** Each lane/side's 3 positions are ordered `[T1, T2, T3]` = farthest-from-that-side's-own-base to closest — a real bug (Dire's top and bot lanes had this backwards, drawing Dire's outermost tower near its own base instead of near the enemy's) shipped and was caught only by the owner visually inspecting the live map, not by any test, because the original tests checked marker *count* and *destroyed-state count*, never *position*. `TOWER_POSITIONS`/`BASE_POSITIONS` are now exported specifically so `dota-minimap.test.jsx`'s geometry regression tests can assert T1 is farther from a side's own base than T3, per lane per side — any future coordinate change must keep passing those.
- Base corners: a soft two-ring circle (outer wide/faint + inner solid-bordered, still flat fills) plus a `RAD`/`DIRE` text label (not just color) — small, but real text, not color-only signaling.
