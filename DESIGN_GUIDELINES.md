...
- **Placement (2026-07-30): directly under the momentum band, above the tower map and `LiveGoldGraph`.** Originally sat below the graph, where it required scrolling past the map and net-worth history to see — moved up so the kill score is visible without scrolling, next to the other "current state" reads (stakes chip, momentum) rather than after the "history" surfaces. The trailing clock line inside this block (`{clock && !momentum && ...}`) only renders when the momentum band above it is null (e.g. gameTime present but net-worth lead isn't finite yet) — otherwise the momentum band's own inline "game time MM:SS" already shows it, and showing both would duplicate the same clock two lines apart.
