...
- **Phase C — Decoder + read API (2026-07-24, shape widened 2026-07-25).** `api/_buildingState.js` (pure `decodeBuildingState`) wired into `liveGamePulse.js` as `pulse.objectives`, gated behind `isOwner` the same way `pulse.history` already is. Shipped as an aggregate `{ rt, dt }` (total standing towers per side); widened the next day to per-lane `{ radiant: [top,mid,bot], dire: [top,mid,bot] }` once Phase D's tower map needed to know which lane, not just a total.
