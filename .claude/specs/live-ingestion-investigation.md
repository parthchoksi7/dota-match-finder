...
**Second cross-check — 2026-08-06, same day, by a stronger method.** Match `8930594356` surfaced while investigating buyback candidates. Rather than waiting for another single diffed event, its raw captured `tower_state`/`barracks_state` (current state, not a diff) was checked bit-by-bit against OD's *cumulative* `objectives[]` history up to that game_time — **32 of 32 bit positions agreed** (9 lane towers + 1 tier-4 pair-count + 6 barracks, ×2 sides). This also surfaced and fixed a real gap: OD's key for a tier-4 (base-guardian) tower kill is the bare `npc_dota_badguys_tower4` (no lane suffix, unlike tier 1-3), which the original `parseOdBuildingObjective` regex silently dropped — a real tier-4 event could never have shown `confirmed` even with matching ground truth. Fixed; OD's own combat log does not distinguish which of the two tier-4 towers fell (both produce the identical key), which is a hard ceiling on what this crosscheck can verify for tier 4 specifically, not a parser gap.

Two matches in, one confirmed by a stronger method than the first. `laneVerified` stays `false` and building events stay `uncertain` until the third lands — the bar is accumulated confidence, not a box to check. Track the running count here as future admin-verification sessions add cross-checked matches:

| # | Match | Method | Result |
|---|---|---|---|
| 1 | `8931981851` (RE.Arise vs No Hoodwink, EPL Masters S1, 2026-08-06) | Single diffed event vs. OD objectives | 1/1 TowerDestroyed confirmed (Radiant mid t1) |
| 2 | `8930594356` (2026-08-06) | State-consistency: every bit vs. OD cumulative history | 32/32 bit positions agreed |

**E13 — Ability↔player mapping** *(needed for `AbilityLearned`)* — **VERIFIED FEASIBLE 2026-08-06.** `abilities[]` are team-level arrays, not nested per-player like items — confirmed, and confirmed tractable: naive attribution (does the ability_id appear in a hero's own innate kit?) resolves only 29.9% of real entries, because the array also carries universal abilities (Glyph/Scan/Roshan-capture/Dota-Plus-cosmetic) and hero talents, neither of which live in a hero's base ability list. Excluding 10 empirically-identified universal ids (each appeared exactly once per side-instance regardless of hero composition) and including each hero's talent tree: **344/347 relevant entries (99.1%) uniquely attributed, 1 collision (0.3%), 2 unattributed (0.6%)**, against 44 real live games. Zero cross-hero collisions in the raw pass before filtering (940/940) — an id, when it resolves at all, has never resolved to more than one hero. Implemented and tested (`buildAbilityOwnerSets`/`attributeAbility` in `api/_liveStoryDiff.js`), not wired into the live differ output — `AbilityLearned` remains out of MVP scope by product decision, not data availability. Full writeup: `.claude/specs/live-story-valve-data-audit.md`.
