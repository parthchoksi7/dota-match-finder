...
|---|---|---|---|---|---|---|
| 23 | Mount `ErrorBoundary` — it's fully built but never rendered | 2 | 3 | 90% | 0.25 | **21.6** |
...
### 23. Mount `ErrorBoundary` — it's fully built but never rendered
- **What:** Found while wiring up Sentry (#7) — `src/ErrorBoundary.jsx` is a complete, working React error boundary (catches render errors, shows a "Something went wrong" + Reload UI) but `grep -rn "ErrorBoundary" src/*.jsx src/main.jsx` (excluding its own file) returns zero matches. Nothing imports or renders it. An uncaught render exception anywhere in the tree today unmounts the whole app to a blank white page instead of the recovery UI that already exists for exactly this case.
- **Fix:** Wrap `<Root />` in `<ErrorBoundary>` in `src/main.jsx`. While touching it, have `componentDidCatch` also call `Sentry.captureException(error, { extra: info })` (Sentry is now initialized in `main.jsx` as of #7) instead of only `console.error` — a caught render crash is exactly the kind of error Sentry should see.
- **Risk:** None — purely additive; today's behavior (React's default unmount-to-blank-page) can only get better.
