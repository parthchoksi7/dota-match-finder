...
| 2 | `fetchMatchSummary` still fetches OpenDota directly from browser | 5 | 5 | 95% | 2 | **11.9** |
| 3 | Mobile touch target audit | 4 | 2 | 60% | 0.5 | **9.6** |
| 4 | VOD pre-fetch in background | 4 | 2 | 80% | 1 | **6.4** |
| 5 | Add Sentry error monitoring | 5 | 4 | 90% | 3 | **6.0** |
| 6 | `seriesMatchMap` key collision on null/0/undefined `seriesId` | 2 | 3 | 90% | 1 | **5.4** |
| 6 | Duplicated `HeroIcon` + `LGM_WINDOW_S` | 3 | 2 | 90% | 1 | **5.4** |
...
### 2. `fetchMatchSummary` still fetches OpenDota directly from the browser
- **Files:** `src/api.js`, `api/summarize.js`
- **What:** Found 2026-07-17 while fixing the `fetchHeroes()` CORS/CSP bug (heroes now proxied via `?mode=heroes-proxy`, commit `6e241d4`). `fetchMatchSummary(matchId)` still does `fetch(OPENDOTA_BASE + '/matches/' + matchId)` directly from the browser, then POSTs the full `matchData` blob to `/api/summarize`. Same exposure class: OpenDota's Cloudflare bot protection can 403 direct browser requests and drop the CORS header, which browsers report as a CORS failure — this would break the AI Summary feature (`App.jsx:776`) the exact same way heroes just broke, and is also blocked outright if the `connect-src` CSP (currently report-only, `vercel.json`) is ever enforced. Fix mirrors the pattern `api/summarize.js` already uses for hero names (`getHeroNames()` — server-side fetch + KV cache): change the client to POST `{ matchId }` instead of `{ matchData }`, and have `api/summarize.js` fetch OpenDota `/matches/{id}` server-side (with its own error handling — `trimMatchDataForSummary` currently assumes a valid object came in).
- **Why it's #2:** Not hypothetical — this is the identical bug class that already took down hero icons/names sitewide days ago. High confidence because the fix pattern is already proven in this codebase; low-ish effort for the same reason.
...
### 6. `seriesMatchMap` can overwrite (not merge) on colliding null/0/undefined `seriesId` keys
...
### 6. Duplicated `HeroIcon` + `LGM_WINDOW_S` across live-series-companion files
...
### 9. Remove dead "Multiple channels were live" copy in MatchDrawer
