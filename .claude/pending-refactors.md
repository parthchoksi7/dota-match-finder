...
### Verify "OG" and "1win Team" PS↔OD name mapping when next active
- **What:** 2026-07-07 tier-1 team-name scrub (see CONTEXT.md `TEAM_NAME_ALIAS_GROUPS`) couldn't confirm two entries: (1) PandaScore's team search for "OG" only returns noise (2-char query matches too many unrelated names) — never found their real PS team id; (2) the new "1win Team" org (inherited Tundra Esports' roster June 2026) has a PandaScore team named "1win" but its match history predates the roster change by months, so it's unclear whether OpenDota will tag 1win's games under a new team_id/name or keep the old "Tundra Esports" identity (OD ties team_id to Steam group continuity, not org branding). Revisit once either team has a live/recent match to check both providers' actual match-time naming — add to `TEAM_NAME_ALIAS_GROUPS` only if a real divergence shows up.
...
### Match-drawer game switcher groups by raw seriesId, unlike groupIntoSeries
- **What:** `src/App.jsx` builds `seriesMatchMap`/`matchGameNumbers`/`seriesGames` by grouping `allMatches` purely on `m.seriesId`. `groupIntoSeries` in `src/utils.js` (used by the home feed / MatchCard) does more: it merges null/0-seriesId orphan games and merges numbered stubs that OpenDota split across different series_ids for the same real series (2nd/3rd passes). So for an OD-split series, the drawer's Game 1/Game 2 switcher can still bucket sibling games separately and mislabel them (each shown as "Game 1"), even though the card view groups them correctly. The 2026-07-08 fix only corrected the *ordering within a bucket* (extracted `orderSeriesGames`); the *bucketing* is still a second, thinner implementation. Unify the drawer's series grouping onto `groupIntoSeries` so both surfaces agree. Note: `groupIntoSeries` also drops the oldest incomplete series, so a naive swap needs care to still surface in-progress series in the drawer.
- **Effort:** Medium
