...
### URL query-param rewrite boilerplate duplicated four times in App.jsx
...
### `fetchMatchSummary` still fetches OpenDota directly from the browser (same CORS exposure just fixed for heroes)
- **What:** Found 2026-07-17 while fixing the `fetchHeroes()` CORS/CSP bug (heroes now proxied via `?mode=heroes-proxy`). `fetchMatchSummary(matchId)` in `src/api.js` still does `fetch(OPENDOTA_BASE + '/matches/' + matchId)` directly from the browser, then POSTs the full `matchData` blob to `/api/summarize`. This is the exact same exposure class: OpenDota's Cloudflare bot protection can 403 direct browser requests and drop the CORS header, which browsers report as a CORS failure — this would break the AI Summary feature (`App.jsx:776`) the same way heroes just broke, and is also blocked outright if the `connect-src` CSP (currently report-only, `vercel.json`) is ever enforced. Fix mirrors the pattern `api/summarize.js` ALREADY uses for hero names (`getHeroNames()` — server-side fetch + KV cache): change the client to POST `{ matchId }` instead of `{ matchData }`, and have `api/summarize.js` fetch OpenDota `/matches/{id}` server-side (with its own error handling — currently `trimMatchDataForSummary` assumes a valid object came in). Not fixed inline because it touches the AI Summary request/response contract, a different feature surface than what was reported.
- **Effort:** Medium (touches `api/summarize.js`'s request contract + `fetchMatchSummary`'s caller in `App.jsx`)
