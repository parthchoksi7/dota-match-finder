...
- ⚠️ **Letter-spacing in tables**: Use `tracking-wide` (not `tracking-widest`) for `text-xs` column headers inside fixed-width columns. `tracking-widest` adds ~3 px per character gap which pushes short labels like "Picks" or "Win%" beyond a `w-10` (40 px) column boundary and causes visible overlap with adjacent cells. *(DraftDisplay.jsx still uses `tracking-widest` on column headers)*
...
- ⚠️ **Filters**: After search, filters like “Has VOD” or “BO5 only” could narrow results without a new query. *(Series type filter built — All/BO1/BO3/BO5; “Has VOD” filter not yet built)*

---

## Summary

| Priority | Area              | Example change                          | Status |
|----------|-------------------|-----------------------------------------|--------|
| High     | Loading           | Initial load skeleton or spinner        | ✅ Built |
| High     | Errors            | Retry on “Failed to load matches”       | ✅ Built |
| High     | A11y              | Focus styles, aria-live, labels         | ✅ Built |
| Medium   | Empty states      | Clear search, retry, short explanations | ✅ Built |
| Medium   | Selected match    | Sticky panel, copy VOD link             | ⚠️ Copy link built; sticky panel not |
| Medium   | Mobile            | Touch target size, footer wrapping      | Not built |
| Low      | Discovery         | Recent/popular searches, BO3/BO5 label  | ⚠️ BO3/BO5 label built; recent searches not |
| Low      | Polish            | Duration format, pagination, theme      | ✅ Built |
...
**Remaining work**: sticky Now Watching panel, `font-display: swap` for fonts, VOD pre-fetch in background, recent/popular search suggestions, “Has VOD” filter, `aria-describedby` on search errors, mobile touch target audit.
