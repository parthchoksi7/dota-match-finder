...
- `src/api.js` - All API calls: OpenDota, Twitch VOD search, hero fetching, match summaries. `fetchMatchIndicators(matchIds)` fetches game indicators (rapier/gold swing/mega comeback/rampage) from `/api/tournaments?mode=match-indicators&ids=...`; results are cached in a module-level `Map` for the browser session so repeated calls (series row mounts, card expands, drawer opens) hit memory instead of the network. `fetchProMatches()` fetches PandaScore recent-completed games in parallel with OpenDota promatches (initial load only) via `/api/tournaments?mode=recent-completed` and merges them using a 3-case algorithm: (1) all PS games already in OD → skip; (2) none in OD → inject all with PS seriesId; (3) partial coverage → inject missing PS games with seriesId bridged from the matching OD game so `groupIntoSeries` groups them together. Extra dedup: when all PS games in a group have `_tempId: true` (no real OD match IDs), two checks apply: (a) time backstop — if all PS games are older than 8h they are dropped (OD indexes within 1-4h so stale PS data is discarded), (b) fuzzy team-pair match — if OD already has a game with matching team names within the last 8h, the PS group is skipped entirely. Team name matching uses bidirectional substring matching (`x.includes(y) || y.includes(x)`) — the same pattern as `teamsMatch()` in `api/match-streams.js` (`psTeamsMatchOd()` in `src/api.js`). This handles team name variations like "Aurora" (PS) vs "Aurora Gaming" (OD) and "BetBoom" (PS) vs "BetBoom Team" (OD). **Authoritative PS↔OD matching pattern**: always use bidirectional substring logic from `api/match-streams.js` `teamsMatch()` — never create a separate algorithm. The 8h window prevents same-team-pair matches from different days being suppressed. PS-sourced games have `_fromPandaScore: true` (drawer shows "STATS PENDING" instead of kill scores), `_tempId: true` when using fallback IDs, and `_pandaMatchId` (merge bookkeeping only). Kill scores are not available from PandaScore; the series record (2-1 etc.) still renders correctly in CompactSeriesRow because it uses `radiantWin` booleans. Once OpenDota indexes the games, the next pull-to-refresh replaces PS-sourced entries with OD entries. `handleLoadMore` in `App.jsx` deduplicates to prevent OD's eventual indexing from adding duplicate entries.
