...
- Manifest: `name="Spectate Esports"`, `short_name="Spectate"`, `display="standalone"`, `theme_color` and `background_color` `#030712` (gray-950 - matches the dark site surface). Icons: `/pwa-192.png`, `/pwa-512.png` (regular, logo at 85% safe area), and `/pwa-maskable-512.png` (Android adaptive icon, logo at 70% safe area to survive the system mask). All three are generated by compositing `public/favicon.png` (the transparent shield) onto an opaque `#030712` square; this matters because iOS and Android render transparent app icons against a system-default white background, which looked broken before. Regenerate via the inline script in the PWA commit history (uses `sharp`, already a transitive dep).
