...
- `src/components/LatestMatches.jsx` - Homepage latest results. Renders a `DateStrip` for date navigation and groups complete series by tournament within the selected date. Each tournament section has a collapsible header (chevron toggles rows; tournament name links to `/tournaments`). Uses `CompactSeriesRow` for each series instead of expandable MatchCards. Derives available dates from series `startTime` values; defaults to the most recent date.
- `src/components/DateStrip.jsx` - Horizontal scrollable date pill strip. Props: `dates` (array of `{key, label}`), `activeDate` (string key), `onChange` (callback). Fires `date_strip_click` GA event. Hidden when fewer than 2 dates are available.
- `src/components/CompactSeriesRow.jsx` - Single-row livescore-style series result. Props: `series`, `onSelectGame`, `spoilerFree`, `followedTeams`, `onToggleFollow`, `isGrandFinal`. Grid layout: team-a | score | team-b | replay-btn (sm+). Clicking the row opens MatchDrawer on the last played game. Replay button opens on Game 1 with `_skipExpand: true`. Winner name is `font-black text-white`; loser is muted gray. Grand Final rows have `border-l-2 border-amber-500/70` left accent. Fires `compact_row_click` and `compact_replay_click` GA events.
...
- `src/pages/PreviewPage.jsx` - **Internal** design preview at `/preview`; NOT indexed (disallowed in robots.txt); supports dark/light mode via Tailwind `dark:` variants throughout (toggled by header sun/moon button, persisted to localStorage key `"theme"` same as homepage), state-sectioned feed (Live Now / Coming Up / Results), mobile bottom nav. Fetches same data as homepage: `/api/live-matches`, `/api/upcoming-matches`, `fetchProMatches`. Intentionally diverges from DESIGN_GUIDELINES (flat list rows, large display typography) as a design exploration. Follow/manage teams fully wired: star icons on hover next to team names, ManageTeamsModal accessible from header star button, persisted to localStorage. Results section uses the shared `LatestMatches` component (date strip + tournament-grouped compact rows via `CompactSeriesRow`). Series-centric drawer: clicking a compact row opens the series via `handleOpenSeries(series)`, which sets `selectedSeries` + `selectedGameIndex` state and calls `handleSelectMatch` on the first game. `handleSwitchGame(game, idx)` switches the active game inside the open drawer (reloads VOD + clears summary for the new game). `MatchDrawer` receives a `gameSwitcher` node (G1/G2/G3 tab buttons) rendered below its header when the series has multiple games. In non-spoiler mode each tab shows the game number + winner name; in spoiler mode shows only "G1 / G2 / G3". "Today at a Glance" strip above the results section shows today's completed series using `dayGroups[0]` (derived locally, not from LatestMatches). Old components removed: `ResultCard`, `DateHeader`, `CardSkeleton`, `abbrevTournament`, starred-only filter.
