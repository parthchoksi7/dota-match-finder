...
- Game tweet prompt uses a social media manager persona; rules enforce: strong hook in first 5 words, game-number-aware angle (Game 1 = set narrative, Game 2 2-0 = shock/momentum, Game 2 1-1 = tease decider, deciding game = drama), natural Dota 2 vocabulary (buyback, aegis, rax, etc.) only where it fits, never start with the winner's name, banned generic win verbs (dominated/demolished/steamrolled etc.), 1-2 emojis allowed if they add energy
- Series tweet prompt uses same persona; first line must be exactly "{winner} {score} {loser}" with no changes; commentary frames the narrative (upset? dominant run? close fight?) using Dota 2 scene context
