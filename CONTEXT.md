...
- `src/components/SiteHeader.jsx` - Shared site header used by all pages. Minimal first-principles design (May 2026 redesign): logo + tagline, one text link (Tournaments, hidden below `md:` because mobile uses BottomTabBar), Spoiler-free toggle (when `onSpoilerToggle` prop is passed), and a Settings cog button that dispatches `SETTINGS_OPEN_EVENT`. Renders `<SettingsSheet />` so the sheet is available wherever the header is. No longer manages theme directly - that moved into SettingsSheet.
...
- **SiteHeader (every page)**: logo + tagline, one Tournaments text link (desktop only via `md:inline`), Spoiler-free toggle (when applicable), Settings cog. Total max 4 items.
...
- **SettingsSheet (everywhere)**: a slide-up sheet (mobile) / dropdown (desktop) holding Theme, Calendar, Install, About, What's New. Triggered via `SETTINGS_OPEN_EVENT` window event - both the header cog and the bottom-bar More tab dispatch this. Decoupled, no prop drilling.
