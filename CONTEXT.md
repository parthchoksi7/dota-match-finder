...
- `api/sitemap.js` - Generates `/sitemap.xml` with slug URLs for recent Tier 1 matches; cached at edge for 1h. Also fetches `/api/tournaments?mode=series` to include `/tournament/:id` URLs for all running (priority 0.9/hourly), upcoming (0.8/daily), and up to 10 most recent completed (0.6/never) tournaments. Includes `/llms.txt` as a sitemap entry. Tournament fetch is wrapped in try/catch — if it fails, match URLs still return normally.
