...
- `src/components/HomeFeed.jsx` - **Main homepage feed.** Mobile-first. Props: `liveMatches`, `upcomingMatches`, `allMatches`, `onSelectMatch`, `onSelectSeries`, `spoilerFree`, `followedTeams`, `onToggleFollow`, `grandFinalMatchIds`, `error`, `onRetry`, `onSelectMatchId`, `tournamentIdMap`, `onLoadMore`, `loadingMore`, `hasMore`. Internal state: `activeDate`, `activeFilter` ('all'|'live'|'upcoming'|'completed'), `expandedTournamentName`. No collapse/expand of match rows — all rows always visible. Tournament card header is a full-width button that expands TournamentHub inline (above match rows) or navigates to /tournaments if no hub ID found. A "My Teams" amber card appears at the very top of the date feed when followed teams have matches on the active date. Followed-team rows in all states (live/upcoming/completed) get amber left border and are sorted to the top within their card. Filter bar includes a persistent calendar icon button (right edge) linking to /calendar. DateStrip receives `onLoadEarlier` and `loadingEarlier` props to render the "Earlier" pill. Fires `feed_filter`, `tournament_hub_expand`, `calendar_icon_click` GA events.
...
- `src/components/DateStrip.jsx` - Horizontal scrollable date pill strip. Props: `dates`, `activeDate`, `onChange`, `onLoadEarlier`, `loadingEarlier`. When `onLoadEarlier` is provided, renders a "← Earlier" pill as the leftmost item that loads older matches (replaces the old bottom "Load more" button). Auto-scrolls active pill into view on mount. Fires `date_strip_click` and `load_earlier_click` GA events. Hidden when fewer than 2 dates are available.
