...
- `src/components/InstallPrompt.jsx` renders a fixed bottom banner. On Android Chrome, listens for `beforeinstallprompt`, shows an Install button that triggers the native dialog. On iOS Safari (UA sniffed), shows a static hint: "Tap Share then Add to Home Screen". Hidden when `matchMedia('(display-mode: standalone)')` is true (already installed). Auto-shows on first visit; suppressed by `localStorage['pwa-install-dismissed']='1'` after the user dismisses. Also listens for a custom `pwa-show-prompt` window event to manually re-show the banner regardless of dismiss flag - dispatched by the "Install as app" row in `SettingsSheet` (the persistent install entry point after the May 2026 nav redesign). Exports `SHOW_EVENT = 'pwa-show-prompt'`. Mounted in `App.jsx` near the top of the main render. Fires GA events: `pwa_prompt_show` (with `trigger: 'auto' | 'manual'`), `pwa_prompt_dismiss`, `pwa_install_click`, `pwa_install_outcome` (with outcome `accepted`/`dismissed`), `pwa_install_icon_click` (now sourced from `settings_sheet`).
- When the manual trigger fires and no deferred prompt is available (e.g. Firefox, Chrome before `beforeinstallprompt` event has fired, or other non-installable browsers), the banner falls back to a generic hint: 'Open your browser menu and tap "Install app"'.
