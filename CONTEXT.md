...
- `SiteHeader.jsx` includes a download icon button (Feather download SVG) between Calendar and Spoiler. Hidden when `matchMedia('(display-mode: standalone)')` is true. On click: dispatches the `pwa-show-prompt` event and tracks `pwa_install_icon_click`. Provides a persistent way to install after the auto-banner has been dismissed.
