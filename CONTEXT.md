...
- `src/components/DateStrip.jsx` - Compact arrow-nav date selector. Props: `dates`, `activeDate`, `onChange`, `onLoadEarlier`, `loadingEarlier`. Renders a single row: left chevron button / current date label (centered) / right chevron button. Left arrow navigates to the previous date in the `dates` array; when at the oldest date it triggers `onLoadEarlier` instead. Right arrow navigates to the next date; disabled when on the newest date. Loading spinner replaces left arrow while `loadingEarlier` is true. Hidden when `dates` is empty. Fires `date_nav_prev`, `date_nav_next`, and `load_earlier_click` GA events.
