...
- `src/components/InstallPrompt.jsx` renders a fixed bottom banner. On Android Chrome, listens for `beforeinstallprompt`, shows an Install button that triggers the native dialog. On iOS Safari (UA sniffed), shows a static hint: "Tap Share then Add to Home Screen". Hidden when `matchMedia('(display-mode: standalone)')` is true (already installed). Auto-shows on first visit; suppressed by `localStorage['pwa-install-dismissed']='1'` after the user dismisses. Also listens for a custom `pwa-show-prompt` window event to manually re-show the banner regardless of dismiss flag - dispatched by the download icon in `SiteHeader`. Exports `SHOW_EVENT = 'pwa-show-prompt'` for that callsite. Mounted in `App.jsx` near the top of the main render. Fires GA events: `pwa_prompt_show` (with `trigger: 'auto' | 'manual'`), `pwa_prompt_dismiss`, `pwa_install_click`, `pwa_install_outcome` (with outcome `accepted`/`dismissed`), `pwa_install_icon_click` (from header icon).
- `SiteHeader.jsx` includes a download icon button (Feather download SVG) between Calendar and Spoiler. Hidden when (a) `matchMedia('(display-mode: standalone)')` is true (already installed) OR (b) `matchMedia('(hover: hover) and (pointer: fine)')` is true (mouse-equipped desktop, where home-screen install is rare and clutters the header). Touch tablets (iPad etc.) still see the icon. On click: dispatches the `pwa-show-prompt` event and tracks `pwa_install_icon_click`. Provides a persistent way to install after the auto-banner has been dismissed. The header nav uses `flex flex-wrap` so items wrap to a second line on tight viewports rather than overflowing off-screen.
