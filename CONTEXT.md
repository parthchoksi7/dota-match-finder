...
- `src/components/InstallPrompt.jsx` manages the install experience per platform. Detects iOS Safari (`/safari/i` + not CriOS/Chrome), iOS Chrome (`/crios/i`), and Android. **iOS Safari**: auto-shows a 3-step modal guide on first visit ("Tap Share button", "Add to Home Screen", "Tap Add"); re-shows on manual `SHOW_EVENT`. **iOS Chrome**: shows "Open in Safari to Install" tip with a Copy Link button when manually triggered (auto-show skipped since Chrome cannot install PWAs). **Android**: listens for `beforeinstallprompt`; shows a bottom banner with a native Install button; falls back to "Open browser menu" hint if event hasn't fired. Hidden when `matchMedia('(display-mode: standalone)')` is true (already installed). Dismiss persists to `localStorage['pwa-install-dismissed']='1'`. Exports `SHOW_EVENT = 'pwa-show-prompt'`. Fires GA events: `pwa_prompt_show` (with `platform` and `trigger: 'auto'|'manual'`), `pwa_prompt_dismiss`, `pwa_install_click`, `pwa_install_outcome`, `pwa_copy_link_ios_chrome`.
