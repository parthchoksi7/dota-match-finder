...
- `src/App.jsx` - Main app, state management, search, load more, drawer, spoiler-free toggle, slug URL generation. Fetches live/upcoming data from `/api/live-matches` and `/api/upcoming-matches` on mount and polls every 2 min. Renders a sticky live banner below the header when matches are in progress. Non-search homepage renders: CalendarNudge → HomeFeed. No separate MyTeamsSection or TournamentHub chips — both are integrated into HomeFeed. `tournamentIdMap` (Map&lt;name, id&gt;) built from `/api/tournaments` (ongoing + upcoming + completed) and passed to HomeFeed for inline hub expansion. Search is a full-screen overlay toggled by `searchOpen` state; the search icon in SiteHeader opens it. `?q=` URL param sets the query and also calls `setSearchOpen(true)` on mount. `handleOpenSeries` always opens `series.games[0]` (Game 1) -- this ensures the PandaScore stream lookup uses G1's startTime (closest to the series `begin_at`), which maximises VOD link reliability on first open.
- `src/main.jsx` - Entry point; path-based routing: `/about` -> AboutPage, `/release-notes` -> ReleaseNotesPage, `/calendar` -> Calendar, else App
- `src/api.js` - All API calls: OpenDota, Twitch VOD search, hero fetching, match summaries. `fetchMatchStats(matchId)` fetches end-game stats for a single match from `/api/tournaments?mode=match-stats&id={matchId}` — returns `{ radiantGoldAdv, players, itemNames }` or null on failure. Module-level `_statsCache` Map prevents redundant fetches on drawer re-opens (same pattern as `_indicatorsCache`). `fetchMatchIndicators(matchIds)` fetches game indicators (rapier/gold swing/mega comeback/rampage) from `/api/tournaments?mode=match-indicators&ids=...`; results are cached in a module-level `Map` for the browser session so repeated calls (series row mounts, card expands, drawer opens) hit memory instead of the network. `fetchProMatches()` fetches PandaScore recent-completed games in parallel with OpenDota promatches (initial load only) via `/api/tournaments?mode=recent-completed` and merges them using `mergeWithPsGames()` — a 3-case algorithm: (1) all PS games already in OD by match ID → skip (OD is authoritative, has kill scores); (2) none in OD → inject all (truly not yet indexed); (3) partial coverage → inject missing PS games with seriesId bridged from the matching OD game so `groupIntoSeries` groups them together. **Backend resolves real OD match IDs** (via timestamp lookup in `findOdMatchByTime`) so Case 1 fires for indexed matches without any frontend name matching — the BB/BetBoom dedup problem is solved at the source. When all PS games in a group have `_tempId: true` (backend couldn't resolve a real OD ID), a single check applies: time backstop — if all PS games are older than 8h they are dropped (OD indexes within 1-4h so stale PS data is discarded). No frontend fuzzy team-pair matching; the backend is the canonical place for PS↔OD resolution. PS-sourced games have `_fromPandaScore: true` (drawer shows "STATS PENDING" instead of kill scores), `_tempId: true` when using fallback IDs, and `_pandaMatchId` (merge bookkeeping only). Kill scores are not available from PandaScore; the series record (2-1 etc.) still renders correctly in CompactSeriesRow because it uses `radiantWin` booleans. Once OpenDota indexes the games, the next pull-to-refresh replaces PS-sourced entries with OD entries (Case 1 fires, PS entry skipped). `handleLoadMore` in `App.jsx` deduplicates to prevent OD's eventual indexing from adding duplicate entries.
...
- `src/components/GoldGraph.jsx` - Custom SVG gold advantage chart. Props: `radiantGoldAdv: number[]` (per-minute array from OpenDota), `radiantName: string`, `direName: string`, `loading?: boolean`. Uses two `<path>` elements with `<clipPath>` to render green fill above the zero line (radiant ahead) and red fill below (dire ahead) — no chart library. `computePoints(data)` is exported for unit tests; it maps the array to SVG `{x, y}` coordinates using viewBox constants (VW=480, VH=140, PL=40, PR=54, PT=14, PB=22). Loading state: `animate-pulse` skeleton. Empty/unavailable state (fewer than 2 data points): "Gold data unavailable" text. Time axis labels every 5 minutes, "RADIANT"/"DIRE" Y-axis labels, final gold value at right edge formatted as `+12.3k`. Spoiler-free and PandaScore guards are applied by the parent (MatchDrawer).
- `src/components/ItemSlot.jsx` - Single item icon slot. Props: `itemId: number`, `itemNames: { [id]: name }`, `size?: 'sm' | 'md'`. If itemId is 0 or the name is not in the map, renders a gray empty-slot placeholder. Otherwise renders `<img src="https://cdn.dota2.com/apps/dota2/images/items/{name}_lg.png" />` with `loading="lazy"` and an `onError` fallback to the empty placeholder. Sizes: sm = `w-5 h-5`, md = `w-6 h-6` (default).
...
- `src/components/SiteHeader.jsx` - Shared site header used by all pages. Minimal first-principles design (May 2026 redesign): logo-only on mobile (wordmark/tagline hidden below `sm:`), full logo + wordmark + tagline on desktop. Tournaments text link desktop-only (`hidden md:inline`). Spoiler-free toggle when `onSpoilerToggle` prop is passed. Settings cog button (`hidden md:inline-flex`) desktop-only - on mobile the More tab in BottomTabBar is the sole entry point to SettingsSheet. Renders `<SettingsSheet />` so the sheet is available wherever the header is.
...
- `src/components/BottomTabBar.jsx` - Fixed-bottom tab bar shown on mobile only (`md:hidden`). Three tabs: Home (`/`), Tournaments (`/tournaments`), More (dispatches `SETTINGS_OPEN_EVENT`). Active state: `text-red-500`. Active tab determined by `window.location.pathname` (works because the app uses full-page navigation, not client-side routing). Includes safe-area inset padding for iPhone notch. Fires `bottom_nav_tap` GA event with `destination` field.
...
- `src/components/TournamentHub.jsx` - Tournament section with Overview/Standings/Schedule/Heroes/Videos tabs, format badge, event stage pipeline, horizontal bracket tree, stage switcher. Accepts `hideStatusLabel` prop: when true, hides the status label row ("Upcoming Tournament"/"Recently Completed") and the header "Add to Calendar" button entirely, and adds `p-3 sm:p-4` padding to the root div — used when expanding inline from HomeFeed. When not in inline mode: section header shows status label + "Add to Calendar" (all-tournaments feed). Per-tournament "Add to calendar" link remains inside the section regardless of mode. Fires `calendar_subscribe_modal_open` with `source: 'tournament_hub_header'` or `'tournament_hub_card'`. Adaptive chip bar for multi-tournament live views via `getTabLabel()`. Fires `tournament_hub_region_select` on chip click.
...
- `src/utils.js` - Series grouping logic (`groupIntoSeries`, `isSeriesComplete`, `winsRequiredForSeries` — all exported); OpenDota `series_type` values: 0=BO1, 1=BO3, 2=BO5, **3=BO2** (undocumented); BO2 draws (1-1 after 2 games) are explicitly marked complete; `getSeriesLabel` maps seriesType 3 → "BO2"; `trackEvent` (dual Vercel + GA4 tracking) — always import this from utils, never redefine locally in components; `toTitleCase(str)` — capitalizes first letter of each word, used for display-layer tournament name formatting (applied in TournamentHub, TournamentBar, Calendar); `getLeagueLabel(name)` — extracts short org label from a tournament name (DreamLeague/ESL/PGL/BLAST/WePlay/Riyadh Masters/The International/Beyond The Summit), used by LatestMatches and TournamentHub for org eyebrow labels; `formatDuration(isoTimeStr)` — converts "H:MM" ISO time string to total minutes (e.g. "1:23" → "83m"); always shows total minutes, never "1h 23m"
...
- Filter is a pure OR of two independent rules — a match is kept if EITHER passes:
  1. **OpenDota premium**: `fetchPremiumLeagueIds()` fetches `/api/leagues` and filters to `tier === 'premium'` leagues only (professional is intentionally excluded; tier A events are covered via rule 2). `premiumIds.has(m.leagueid)` returns true for premium matches.
...
- **SiteHeader (every page)**: logo only on mobile (wordmark hidden below `sm:`), logo + wordmark + tagline on desktop. Tournaments text link desktop-only (`hidden md:inline`). Spoiler-free toggle when applicable. Settings cog desktop-only (`hidden md:inline-flex`) - mobile uses the More tab instead.
- **BottomTabBar (mobile only)**: Home / Tournaments / More tabs in the thumb zone. `md:hidden` hides on desktop. Page main containers use `pb-20 md:pb-8` to avoid content being obscured.
...
- **OpenDota indexing lag**: `/promatches` takes 30min–several hours to index games after a series ends. The PandaScore `?mode=recent-completed` fallback bridges this gap: PS-sourced games appear immediately after a series ends with "STATS PENDING" kill scores (only win/loss is available from PandaScore). Once OD indexes them, the next pull-to-refresh shows full kill scores. Series record (2-1 etc.) is always correct since it uses `radiantWin` booleans, not kill counts.
...
- Recent completed (PandaScore fallback): `https://spectateesports.live/api/tournaments?mode=recent-completed&bust=1`
