...
- **Exact channel matching**: while a match is live on exactly one stream, `api/live-matches.js` stores the channel in KV under `stream:match:{openDotaMatchId}` (7-day TTL). When the user opens a completed game, `api/match-streams.js` is queried first; if a channel is found, Twitch is searched on that channel only, giving a single definitive VOD link
- Falls back to multi-channel search when no stream mapping exists
