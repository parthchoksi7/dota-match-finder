...
- `src/App.jsx` - Main app, state management, search, load more, drawer, spoiler-free toggle, slug URL generation. Fetches live/upcoming data from `/api/live-matches` and `/api/upcoming-matches` on mount and polls every 2 min. Renders a sticky live banner below the header when matches are in progress. Non-search homepage renders: MyTeamsSection (top) → CalendarNudge → TournamentHub chips → HomeFeed. Tournament hub chips (`tournamentPills` state from `/api/tournaments`) show 1-3 live/upcoming events; clicking expands inline TournamentHub via `expandedTournamentId`.
...
- `src/components/HomeFeed.jsx` - **Main homepage feed.** Replaces LatestMatches + UpcomingMatches + TournamentHub on the homepage. Props: `liveMatches`, `upcomingMatches`, `allMatches`, `onSelectMatch`, `onSelectSeries`, `spoilerFree`, `followedTeams`, `onToggleFollow`, `grandFinalMatchIds`, `error`, `onRetry`, `onSelectMatchId`. Internal state: `activeDate` (null = most recent date with data), `activeFilter` ('all'|'live'|'upcoming'|'completed'), `collapsedTournaments`. Builds chronological `availableDates` from completed series + today + tomorrow; defaults to most recent date with data (today if today has content, else most recent past day). Filter bar counts per date. Tournament cards group live + upcoming + completed rows; sorted live-first, then upcoming, then completed. Followed-team completed rows get amber left border via `isFollowedMatch` prop. Fires `feed_filter` and `tournament_card_collapse` GA events.
- `src/components/LiveMatchRow.jsx` - Compact live match row inside HomeFeed tournament cards. Props: `match` (PandaScore shape: `teamA`, `teamB`, `seriesScore`, `currentGame`, `streams`, `tournament`), `onSelectMatchId`, `spoilerFree`, `isFollowedMatch`. Grid: team-a | 80px score | team-b | Watch button (hidden on mobile). Always has red left border (`border-l-red-500`); amber border if `isFollowedMatch`. Pulsing `G{n}` indicator below score when `match.currentGame` is set. Fires `live_match_watch` GA event.
- `src/components/UpcomingMatchRow.jsx` - Compact upcoming match row inside HomeFeed tournament cards. Props: `match` (PandaScore shape: `teamA`, `teamB`, `scheduledAt`, `streams`). Shows team names, countdown via `formatMatchTime(scheduledAt)` from utils, and a stream pill (hidden on mobile). Fires `upcoming_stream_click` GA event.
