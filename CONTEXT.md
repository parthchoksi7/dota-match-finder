...
- `src/components/MatchDrawer.jsx` - Slide-in drawer showing match details, VOD links, draft, AI summary. Header shows `{match.date} · {formatDuration(match.duration)}` (total minutes, e.g. "58m"). Score section is split into two rows: (1) names row — centered `flex-wrap` with radiant name, indicators, star, "—" separator, star, indicators, dire name — no `truncate`, names wrap naturally if long; (2) score row — centered `text-4xl` numbers only. `drawerIndicatorSets` (rapier/goldSwing/megaComeback/rampage Sets) is derived from `gameIndicators` via `useMemo` before the early returns. Accepts `gameSwitcher` prop (any React node) rendered in a row below the header. On the homepage, `App.jsx` builds a G1/G2/G3 segmented control from `seriesMatchMap` and passes it as `gameSwitcher`; each chip shows the game number plus the winning team name (e.g. "G1 Nigma Galaxy") in muted tertiary text; winner names are hidden when `spoilerFree` is true; clicking a chip calls `handleSelectMatch` to switch games. Active chip = `selectedMatch?.id`. Only shown for multi-game series. Follow star buttons for both teams appear inline next to team names in the score row (never in CompactSeriesRow). Props: `followedTeams`, `onToggleFollow` passed from App.jsx.
