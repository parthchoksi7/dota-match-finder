...
- `src/components/GameIndicators.jsx` - Icon indicator chip component for notable in-game events. Three indicators: Divine Rapier (red, sword icon), 20K+ Gold Swing (amber, zigzag chart icon), Mega Creep Comeback (violet, lightning bolt icon). Two variants: `compact` (icon-only 20x20 circle chip with colored background, for series rows and game rows) and `full` (icon + text label pill, for the match drawer). Each chip has a hover tooltip via `group/indicator` scoped Tailwind hover. Returns null when `!indicators` or no active indicators. Never rendered in spoiler-free mode.
