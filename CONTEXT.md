...
- `src/api.js` - All API calls: OpenDota, Twitch VOD search, hero fetching, match summaries. `fetchMatchIndicators(matchIds)` fetches game indicators (rapier/gold swing/mega comeback/rampage) from `/api/tournaments?mode=match-indicators&ids=...`; results are cached in a module-level `Map` for the browser session so repeated calls (series row mounts, card expands, drawer opens) hit memory instead of the network. `fetchProMatches()` fetches PandaScore recent-completed games in parallel with OpenDota promatches (initial load only) via `/api/tournaments?mode=recent-completed` and merges them using a 3-case algorithm: (1) all PS games already in OD → skip; (2) none in OD → inject all with PS seriesId; (3) partial coverage → inject missing PS games with seriesId bridged from the matching OD game so `groupIntoSeries` groups them together. PS-sourced games have `_fromPandaScore: true` (drawer shows "STATS PENDING" instead of kill scores) and `_pandaMatchId` (merge bookkeeping only). Kill scores are not available from PandaScore; the series record (2-1 etc.) still renders correctly in CompactSeriesRow because it uses `radiantWin` booleans. Once OpenDota indexes the games, the next pull-to-refresh replaces PS-sourced entries with OD entries. `handleLoadMore` in `App.jsx` deduplicates to prevent OD's eventual indexing from adding duplicate entries.
...
- `src/components/DateStrip.jsx` - Filled pill track date selector (Sofascore style). Props: `dates`, `activeDate`, `onChange`, `onLoadEarlier`, `loadingEarlier`. Two-zone layout: a fixed `w-8` left chevron button (outside the scroll area) + a `flex-1 overflow-x-auto` pill track. Keeping the button outside the scroll area means new pills loading in never shift its position. The chevron dims to `opacity-30` while `loadingEarlier` is true — no text swap, no spinner. Active date = `bg-white dark:bg-gray-800 shadow-sm text-gray-900 dark:text-white rounded-full`; inactive = plain gray text, no fill. Auto-scrolls the active pill into center on mount via `useRef` + `scrollIntoView({ behavior: 'instant', inline: 'center' })`. Hidden when `dates` is empty. Fires `date_strip_click` (per pill) and `load_earlier_click` GA events.
...
- `api/news.js` - Aggregates Dota 2 headlines from RSS feeds (Dota 2 Official + PCGamesN + Dot Esports). `categoryFilter` receives `(categories, url)` — Dot Esports uses URL-path matching (`/dota`) since their category tags are unreliable. Parses RSS 2.0 and Atom with `fast-xml-parser`. Deduplicates by URL hash (djb2-xor). Tags articles with tier-1 team/tournament entities. Caches 30min in KV under `news:articles:dota2:v1`; never caches empty results. `MAX_AGE_DAYS = 60` (raised from 14 to keep older patch notes and tournament coverage visible during slow news cycles; Dota 2 is a low-volume news game and 14 days was too aggressive). Supports `?game=dota2&limit=20&category=match-result&source=steam-dota2&bust=1`. Response shape: `{ articles: Article[], meta: { sources, fetchedAt, cached, total } }`.
